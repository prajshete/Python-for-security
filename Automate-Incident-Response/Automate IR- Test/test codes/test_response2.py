...
import pandas as pd
import urllib3,requests, ssl, vt, os, asyncio, time, typer, json, smtplib, base64, openpyxl, mimetypes, zipfile
...
def send_email(final_results, out_dir):
    ...
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, _, files in os.walk(out_dir):
            for file in files:
                zipf.write(os.path.join(root, file), arcname=os.path.relpath(os.path.join(root, file), out_dir))
...
def isolate_hosts(final_results):
    print("[+] Isolating Hosts . . ")
...
def response(final_results, out_dir):
    
    print("\n [+] Sending Alert Email . . .")
    send_email(final_results, out_dir)
    isolate_hosts(final_results)
...
#--------------------------------- Response -----------------------------------------------------------
    ...
    response(final_results, out_dir)
...
def build_elasticsearch_query(domains):
    # Build the query string
    query_string = " OR ".join([f"dns.question.name:{domain}" for domain in domains])
    
    # Construct the must_not part of the Elasticsearch query
    must_not_query = query_string
...
excel_path = 'false_positive_domains.xlsx'

# Read domains from Excel
domains = read_false_positive_domains(excel_path)

# Build Elasticsearch query
es_query = build_elasticsearch_query(domains)
